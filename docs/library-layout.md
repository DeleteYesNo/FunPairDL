# 媒體庫版面與 `funlib.json` 約定（v1，2026-09-18 修訂）
...
**舊版面（相容，不再產生）**：`<作品>.alt/`、`.alt1/` 子夾內放 `<作品>.alt.mp4`（hardlink）與 `<作品>.alt[.axis].funscript`。FunLib 會把它們當成同一作品的變體（Label = Alt、Alt 1…）。FunPairDL 的遷移工具會把它們改回平放版面：刪掉 hardlink 影片、腳本改名為 `<作品> (Alt).funscript` 等（`.alt/` 內的影片若不是 hardlink 而是另一個編碼版本，維持獨立資料夾，不遷移）。
...
- 分組規則（沒有 `variants[]` 時）：影片主幹是腳本主幹的前綴且下一字元非英數 → 同一作品；`.alt*/` 子夾 → 上層作品；`No Video/<作品>/` 內以資料夾名分組；媒體庫根目錄直接放的檔案不分組。
- 變體播放：變體自己的檔案優先，缺的軸從主版本補（`inherit_axes` 預設 true）。
- `author`：sidecar 優先；否則列名開頭的 `(Author)` 或 `[Author]` 前綴（跳過 `(ZZ-FREE-0002)` 這類包碼、排除 `(Multi-axis)` `(Unknown)` 等描述性前綴）；沒有就留 `Local Scan`。
...
{"id":"k3f9…","ts":"2026-09-18T10:00:00.000Z","action":"trash","scope":"variant","folderIdx":1,"stamp":"20260918-100000",
 ...
 "paths":["Work/Work (Soft).funscript"],"pair_id":"a1b2c3d4e5f6","source_url":"https://discuss.eroscripts.com/t/some-slug/12345"}
