...
check("x.com profile is non-video", ctx.isNonVideoPath("https://x.com/SomeArtist"), true);
...
// ── _isScriptFilename ──
check("funscript ext", ctx._isScriptFilename("Script Demo 01_2026.funscript"), true);
check("funscript ext (mixed case)", ctx._isScriptFilename("X.FunScript"), true);
check("mp4 is not a script", ctx._isScriptFilename("Robin Mock - Wild.mp4"), false);
...
check("real work name is NOT generic",
  ctx._isGenericSectionName("Robin Mock - Wild demo clip one"), false);
check("decorated real name is NOT generic", ctx._isGenericSectionName("༺༻ Tuna Meal ༺༻"), false);
...
check("escapeAttr passes clean names through",
  ctx.escapeAttr("Robin Mock - Wild.mp4"), "Robin Mock - Wild.mp4");
...
// ── _distributeOrphanScripts: generic "Script" section feeds work sections ──
{
  ...
  const parsedSecs = ctx._distributeOrphanScripts([
    sec("Alpha’s practice / Alpha’s calm practice", ["https://e621.net/posts/1"], []),
    sec("Alpha says hello there / AlphaVR", ["https://e621.net/posts/2"], []),
    sec("Alpha’s practice with the coach. / AlphaDoggy", ["https://e621.net/posts/3"], []),
    sec("Script", [], [
      "Alpha’s_Calm_PracticeVR(8K-H265@60fps).funscript",
  ...
  check("orphan: section 1 gets its VR script", names(parsedSecs[0])[0], "Alpha’s_Calm_PracticeVR(8K-H265@60fps).funscript");
