...
class TestAxisCollision:
    ...
    def test_two_L0_variants_max_plus(self, tmp_path):
        ...
        alt_dir = tmp_path / "Weekday.alt"
        assert alt_dir.is_dir()
        assert (alt_dir / "Weekday.alt.L0.funscript").exists()
        # Hardlinked video
        assert (tmp_path / "Weekday.alt" / "Weekday.alt.mp4").exists()
...
class TestExplicitGroups:
    ...
    def test_two_alts_each_with_own_video(self, tmp_path):
        ...
        pair = _make_pair(str(tmp_path), "Mockyu", [
        ...
        # Main in root
        assert (tmp_path / "Mockyu.mp4").exists()
        assert (tmp_path / "Mockyu.funscript").exists()
        assert (tmp_path / "Mockyu.surge.funscript").exists()
        assert (tmp_path / "Mockyu.pitch.funscript").exists()

        # Alt 1 → .alt/
        alt1 = tmp_path / "Mockyu.alt"
        assert (alt1 / "Mockyu.alt.mp4").exists()
        assert (alt1 / "Mockyu.alt.funscript").exists()
        assert (alt1 / "Mockyu.alt.surge.funscript").exists()
        assert (alt1 / "Mockyu.alt.pitch.funscript").exists()

        # Alt 2 → .alt1/
        alt2 = tmp_path / "Mockyu.alt1"
        assert (alt2 / "Mockyu.alt1.mp4").exists()
        assert (alt2 / "Mockyu.alt1.funscript").exists()
        assert (alt2 / "Mockyu.alt1.surge.funscript").exists()
        assert (alt2 / "Mockyu.alt1.pitch.funscript").exists()

        # Alt videos are NOT hardlinks to Main video
        assert not os.path.samefile(
            str(tmp_path / "Mockyu.mp4"), str(alt1 / "Mockyu.alt.mp4")
        )
        assert not os.path.samefile(
            str(tmp_path / "Mockyu.mp4"), str(alt2 / "Mockyu.alt1.mp4")
        )

        # Inherited multi-axis funscripts ARE hardlinks back to Main
        assert os.path.samefile(
            str(tmp_path / "Mockyu.surge.funscript"),
            str(alt1 / "Mockyu.alt.surge.funscript"),
        )
        assert os.path.samefile(
            str(tmp_path / "Mockyu.surge.funscript"),
            str(alt2 / "Mockyu.alt1.surge.funscript"),
        )

    def test_alt_display_name_drives_subfolder(self, tmp_path):
        """Alt with `display_name` gets its own meaningful subfolder
        name (e.g. `示例风格.alt/`) instead of the bland `Topic.alt/`."""
        _touch(tmp_path / "op.mp4")
        _touch(tmp_path / "op.funscript")
        _touch(tmp_path / "op.surge.funscript")
        _touch(tmp_path / "c1.mp4")
        ...
        pair.alt_group_config = {
            "Alt 1": {"inherit_multi_axis": True, "display_name": "示例风格"},
        }
        ...
        alt_dir = tmp_path / "示例风格.alt"
        assert alt_dir.is_dir()
        # And there's no fallback Mockyu.alt sitting around
        assert not (tmp_path / "Mockyu.alt").exists()

        # Files inside share the same stem
        assert (alt_dir / "示例风格.alt.mp4").exists()
        assert (alt_dir / "示例风格.alt.funscript").exists()
        assert (alt_dir / "示例风格.alt.surge.funscript").exists()
        # Inherited surge is a hardlink to Main
        assert os.path.samefile(
            str(tmp_path / "Mockyu.surge.funscript"),
            str(alt_dir / "示例风格.alt.surge.funscript"),
        )
