...
class TestNormalize:
    def test_apostrophe_variants_match(self):
        ...
        v = normalize("[Infected_Heart] MPOV Jane Doe's Holiday Letter.mp4")
        s = normalize("[Infected_Heart] MPOV Jane Doe s Holiday Letter.funscript")
        assert v == s
...
class TestPairFiles:
    def test_list_siblings_group_in_folder(self):
        ...
        LIST = "list:abc"
        cands = [
            _vid(1, "[LinuxDx] Mocka Squatting.mp4", LIST),
            _scr(2, "[LinuxDx] Mocka Squatting.funscript", LIST),
            _scr(3, "[LinuxDx] Mocka Squatting.pitch.funscript", LIST),
            _scr(4, "[LinuxDx] Mocka Squatting.roll.funscript", LIST),
        ]
    ...
    def test_apostrophe_pair_not_orphaned(self):
        ...
        LIST = "list:abc"
        cands = [
            _vid(1, "[Infected_Heart] MPOV Jane Doe's Holiday Letter.mp4", LIST),
            _scr(2, "[Infected_Heart] MPOV Jane Doe s Holiday Letter.funscript", LIST),
        ]
        groups = pair_files(cands)
        assert len(groups) == 1
        g = groups[0]
