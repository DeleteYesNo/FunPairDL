...
def test_local_tags(tmp_path):
    ...
    assert bf.pack_tag("(Author) Genshin Impact Shenhe 5 Packs! (Total 6m 13s)") == "pack-genshin-impact-shenhe-5-packs-total-6m"


def test_bundle_keys_from_item_urls():
    pair = {"items": [
        {"url": "https://mega.nz/folder/AbCdEf12#keykeykey/file/xyz"},
        {"url": "https://pixeldrain.com/api/filesystem/Root1234/All%202D%20May/x.mp4"},
...
def test_core_title_and_fuzzy_overlap():
    core, short = bf.core_title("(Tail Blazer) Legend of Zelda - Mommy Zelda's HJ")
    assert core == "Legend of Zelda - Mommy Zelda's HJ" and short == "Mommy Zelda's HJ"
    hcore, hshort = bf.core_title("(Tail blazer) Mommy Zelda's HJ (Requested, HQ script)")
    assert hcore == "Mommy Zelda's HJ" and hshort == "Mommy Zelda's HJ"
...
def test_name_tags():
    assert bf.name_tags("bready-nikke-godness-of-victory-uncensored_1080p") == ["source-rule34video"]
    assert bf.name_tags("2b-loop2_1080p60FPS") == ["source-rule34video"]
    assert bf.name_tags("TSMCB VAM HMV 4k") == ["hmv", "3d"]
    assert bf.name_tags("PixelFH-FH-Endurance-Round-2") == ["fap-hero"]
