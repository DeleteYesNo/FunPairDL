...
def test_sanitize_label_drops_brackets_and_path_chars():
    ...
    assert lib.sanitize_label("样本(示例风格)") == "样本 示例风格"
...
def test_alt_dir_label():
    ...
    assert lib.alt_dir_label("Mocka x Demia.alt", "[Keke] Mocka x Demia (+Part2)") == "Mocka x Demia"
    assert lib.alt_dir_label("样本(示例风格).alt", "[甲乙]样本") == "样本 示例风格"


def test_author_from_name_matches_funlib_rule():
    assert lib.author_from_name("(Theobrobine) Mockgame - Lina") == "Theobrobine"
    assert lib.author_from_name("[Wutboi] Mockran [Multi-Axis]") == "Wutboi"
    assert lib.author_from_name("(ZZ-FREE-0002)(Crisisbeat)Weekday") == "Crisisbeat"
...
def test_write_and_read_sidecar_utf8_no_bom(tmp_path):
    p = lib.write_sidecar(tmp_path, {"title": "样本"})
    raw = p.read_bytes()
    assert not raw.startswith(b"\xef\xbb\xbf")
    assert lib.read_sidecar(tmp_path) == {"title": "样本", "version": 2}
...
def test_topic_meta_from_json():
    topic = {"id": 100001, "slug": "fap-hero-demo", "title": "Fap Hero Demo [Quantoz]",
             "tags": [{"name": "Fap Hero", "slug": "fap-hero"}, {"name": "HMV"}, "Multi Axis"],
             "category_id": 14, "created_at": "2026-01-02T03:04:05.678Z",
    ...
    assert m["source"] == {"site": "eroscripts", "topic_id": 100001,
                           "url": "https://discuss.eroscripts.com/t/fap-hero-demo/100001"}
    assert m["tags"] == ["fap-hero", "hmv", "multi-axis"]
    assert m["category"] == {"id": 14, "name": "Free Scripts"}
    assert m["posted_at"] == "2026-01-02T03:04:05.678Z"
