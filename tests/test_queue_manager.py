...
class TestPlanBundleSplit:
    ...
    def test_mirror_on_a_file_host_is_not_split_from_the_source_page(self):
        ...
        pd = PairItem(url="https://pixeldrain.com/u/MkId0103",
...
class TestAutoSplitBundlePair:
    def test_splits_distinct_works(self):
        ...
        names = ["[Gweda] Mockie", "[Teamboobs]Mockie", "[simao] Mockie"]
        items = []
        for n in names:
            items.append(_vi(n + ".mp4", FileType.VIDEO))
            items.append(_vi(n + ".funscript", FileType.FUNSCRIPT))
    ...
    def test_mirror_videos_not_split(self):
        ...
        items = [
            _vi("Mockie.mp4", FileType.VIDEO),
            _vi("Mockie.mp4", FileType.VIDEO),
            _vi("Mockie.funscript", FileType.FUNSCRIPT),
        ]
        assert QueueManager()._auto_split_bundle_pair(Pair(name="Mockie", items=items)) is None
    ...
    def test_real_names_pair_one_to_one(self):
        # Regression for the RobinMock bundle bug: a list of N works, each a
        ...
        works = [
            "Robin Mock - Wild demo clip one",
            "Robin Mock - Second demo clip title",
            "Robin Mock - Third demo clip keeps going",
        ]
        items = []
        for w in works:
    ...
    def test_matches_across_prefix_and_resolution(self):
        ...
        # right work — the MockTea bug orphaned the video and dumped the script
        ...
        items = [
            ...
            # first (longest) pair, so if matching were broken the lunara script
            ...
            _vi("gifts-of-lunara-qzx2_2160p.mp4", FileType.VIDEO),
            _vi("MockElf_4Some.mp4", FileType.VIDEO),
            _vi("(MOCKHEART)gifts-of-lunara-qzx2_1080p.funscript", FileType.FUNSCRIPT),
            _vi("(Left)MockElf_4Some.funscript", FileType.FUNSCRIPT),
        ]
        result = QueueManager()._auto_split_bundle_pair(Pair(name="Vault", items=items))
        assert result is not None and len(result) == 3
        by = {p.name: p for p in result}
        # the lunara script matched its video, not the longest decoy pair
        decoy = by["a-very-long-unrelated-decoy-scene-title-here_2160p"]
        assert all("lunara" not in i.filename.lower() for i in decoy.items)
        bless = by["gifts-of-lunara-qzx2_2160p"]
        drae = by["MockElf_4Some"]
        assert any(i.file_type == FileType.FUNSCRIPT and "lunara" in i.filename.lower()
                   for i in bless.items)
        assert any(i.file_type == FileType.FUNSCRIPT and "mockelf" in i.filename.lower()
                   for i in drae.items)
...
class TestBundleFilenames:
    ...
    def test_grouped_uses_provided_filenames(self):
        ...
        pair = qm.add_pair(
            ...
            groups=[{
                "name": "Main",
                "video_urls": ["https://pixeldrain.com/u/MkId0101"],
                "script_urls": ["https://pixeldrain.com/u/MkId0102"],
                "filenames": {
                    "https://pixeldrain.com/u/MkId0101": "Robin Mock - Wild.mp4",
                    "https://pixeldrain.com/u/MkId0102": "Robin Mock - Wild.funscript",
                },
            }],
        )
        vids = [i for i in pair.items if i.file_type == FileType.VIDEO]
        scrs = [i for i in pair.items if i.file_type == FileType.FUNSCRIPT]
        assert vids[0].filename == "Robin Mock - Wild.mp4"
        assert scrs[0].filename == "Robin Mock - Wild.funscript"

    def test_falls_back_to_url_guess_without_filenames(self):
        qm = QueueManager()
        pair = qm.add_pair(
            name="X",
            groups=[{"name": "Main", "video_urls": ["https://pixeldrain.com/u/MkId0101"]}],
        )
        vids = [i for i in pair.items if i.file_type == FileType.VIDEO]
        assert vids[0].filename == "MkId0101"  # _guess_filename → URL tail
...
class TestAddPairSizes:
    ...
    def test_grouped_sizes_applied(self):
        qm = QueueManager()
        v = "https://pixeldrain.com/u/MkId0101"
        s = "https://pixeldrain.com/u/MkId0102"
        pair = qm.add_pair(
            name="Sized",
            groups=[{
                "name": "Main",
...
class TestCleanTitle:
    def test_bundle_url_detection(self):
        ...
        assert QueueManager._is_bundle_url("https://pixeldrain.com/d/MkId0104")
...
class TestOrganizeDedupOnRedownload:
    ...
    def test_drops_byte_identical_redownload(self, tmp_path):
        ...
        dl = tmp_path / "mock-bunny_1080p.mp4"
        dl.write_bytes(existing.read_bytes())  # identical re-download
        pair = self._video_pair(tmp_path, name, "mock-bunny_1080p.mp4")
